...
ggsave("NMA network 1 year.tiff",width=12,height=4, compression="lzw")
...
plot(NMA.ex1yr,weight_edges=T,weight_nodes=T,layout="star") +
  ggplot2::guides(edge_width = "none", size = "none")
ggsave("NMA network exact 1 year.tiff",width=12,height=4, compression="lzw")
...
plot(relative_effects(nma.run,all_contrasts=TRUE), ref_line=0, .width=c(0,0.95)) +
    scale_x_continuous("Mean difference in logMAR (1st named vs 2nd)")
...
write_csv(all.res.df,"NMA logMAR unadjusted results.csv")

sink("NMA logMAR unadjusted prob rankings.txt")
walk(all.models, get_ranks)
walk(all.models, get_ranks.b)
sink()

sink("NMA logMAR unadjusted all results.txt")
walk(all.models, print)
sink()

filename.set <- c("NMA logMAR 1-year main.tiff", 
                  "NMA logMAR 1-year by class.tiff",
                  "NMA logMAR exact 1-year by class.tiff",
                  "NMA logMAR 1-year by drug.tiff",
                  "NMA logMAR exact 1-year by drug.tiff",
                  "NMA logMAR exact 1-year main.tiff")
...
NMA.adj <- combine_network(set_agd_arm(filter(ad.bcva.tb1,!is.na(logMAR.base)),
                                       study=Trial,
                                       trt=Arm,
                                       y=logMAR.mcfb,
                                       se=logMAR.mcfb.sem,
...
NMA.class.adj <- combine_network(set_agd_arm(filter(ad.bcva.tb1,!is.na(logMAR.base)),
                                       study=Trial,
                                       trt=Class,
                                       y=logMAR.mcfb,
                                       se=logMAR.mcfb.sem,
...
plot(NMA.class.adj,weight_edges=T,weight_nodes=T,layout="star") +
  ggplot2::guides(edge_width = "none", size = "none")
ggsave("NMA network class.tiff",width=12,height=4, compression="lzw")
...
NMA.drug.adj <- combine_network(set_agd_arm(filter(ad.bcva.tb1,!is.na(logMAR.base)),
                                            study=Trial,
                                            trt=Drug,
                                            y=logMAR.mcfb,
                                            se=logMAR.mcfb.sem,
...
plot(NMA.drug.adj,weight_edges=T,weight_nodes=T,layout="star") +
  ggplot2::guides(edge_width = "none", size = "none")
ggsave("NMA network drug.tiff",width=12,height=4, compression="lzw")
...
NMA_res_plot2 <- function(nma.run,filename){
  plot(relative_effects(nma.run,newdata=ref.data,all_contrasts=TRUE), ref_line=0, .width=c(0,0.95)) +
    scale_x_continuous("Mean difference in logMAR (1st named vs 2nd)", breaks=seq(-2,2,0.2))
  ggsave(filename=filename,device="tiff",width=6,height=4,dpi=200,compression="lzw")
}
...
write_csv(all.res.df2,"NMA logMAR adjusted results.csv")

sink("NMA logMAR adjusted prob rankings.txt")
print(walk(all.models2, get_ranks2))
print(walk(all.models2, get_ranks.b2))
sink()

sink("NMA logMAR adjusted all results.txt")
print(walk(all.models2, print))
sink()

filename.set <- c("NMA logMAR adjusted main.tiff", 
                  "NMA logMAR adjusted by class.tiff",
                  "NMA logMAR adjusted by drug.tiff")
